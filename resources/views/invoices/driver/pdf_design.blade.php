            ...
            <td>
                <table id="table" cellspacing="0" cellpadding="5" >
                    ...
                    <tr class="header">
                        ...
                        <th style="line-height: 20px">{{__('tour.driver')}}</th>
                        ...
                        <th>{{__('driver_invoice.driver_license')}}</th>
                    ...
                    @foreach($hires as $hire)
                        <tr class="left">
                            <td style="border-bottom: 0.5px solid #ccc">Hansa Bustouristik {{date("d.m.Y",strtotime($hire->from_date)) }}</td>
                            <td style="border-bottom: 0.5px solid #ccc"> {{ $hire->driver->driver_name }}</td>
                            <td style="border-bottom: 0.5px solid #ccc">Nr. {{ $customer->id  }}</td>
                            <td style="border-bottom: 0.5px solid #ccc">{{ date("H:i:s",strtotime($hire->from_date)) }} Uhr</td>
                            <td style="border-bottom: 0.5px solid #ccc">{{ date("d.m.Y",strtotime($hire->from_date))}}</td>
                            <td style="border-bottom: 0.5px solid #ccc">{{ date("d.m.Y",strtotime($hire->to_date)) }}</td>
                            <td style="border-bottom: 0.5px solid #ccc">{{ $hire->driver->driver_license }}</td>
                            <td style="border-bottom: 0.5px solid #ccc">{{ number_format($hire->price) }} € </td>
                        </tr>
                    @endforeach
                    </tbody>
                    <tfoot>
