                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <select id="customer_id" name="customer_id" onchange="$('#searchForm').submit()"
                            ...
                            <div class="col-md-1.5">
                                <div class="form-group"><input type='text' name="from_date" id="from_date"
                                                               value="{{request()->get('from_date')}}"
                                                               autocomplete="off" placeholder="{{__('tour.from')}}"
                                                               class="form-control datetimepicker1"/></div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    <input type='text' name="to_date" id="to_date" value="{{request()->get('to_date')}}"
                                           autocomplete="off" placeholder="{{__('tour.to')}}"
                                           class="form-control datetimepicker2"/>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group"><input type='text' name="id" id="invoiceID" autocomplete="off"
                                ...
                                <div class="form-group">
                                    <a href="javascript:;" onclick="$('#searchForm').submit()"
                                       class="btn btn-outline-success"><i
                                                class="ft-search"></i> {{__('messages.search')}}</a>
                                </div>
                            </div>
                            @if($request->customer_id)
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group text-left">
                                        ...
                                        <div class="col-md-3">
                                            <label>{{ __('tour_invoice.vat') }}</label>
                                        <input style="width: 64px" type="number" name="vat_bulk" id="vat_bulk"
                                               class="form-control has_numeric_value" value=0 min="0" max="99">
                                        </div>
                                    </div>
                                </div>
                            @else
                        ...
                        <div class="card-body">
                            <div class="px-3 mb-4">
                                ...
                                <div class="table-responsive">
                                    <form class="form" method="POST" action="{{ route('generate-tour-invoice') }}"
                                          id="theForm">
                                        @csrf
                                        ...
                                        <table class="table table-hover table-xl mb-0" id="listingTable">
                                                ...
                                                <th class="border-top-0" width="5%">
                                                    <div class="custom-control custom-checkbox" style="top: -5px;">
                                                        ...
                                                        <label class="custom-control-label"
                                                               for="isSelected">&nbsp;</label>
                                                    </div>
                                                </th>
                                                <th class="border-top-0">Tour #</th>
                                                <th class="border-top-0">{{__('tour.vehicle')}}</th>
                                            ...
                                            @foreach($rows as $row)
                                                <tr>
                                                    <td>
                                                        <div class="custom-control custom-checkbox" style="top: -5px;">
                                                            <input type="checkbox" id="{{$row->id}}"
                                                                   ...
                                                                   class="custom-control-input form-check-input ids"
                                                                   onclick="addTours();" value="{{$row->id}}"
                                                                   name="ids[]">
                                                    ...
                                                    <input type="hidden" id="customer_id_{!! $row->id !!}"
                                                           value="{!! $row->customer_id !!}">
                                                    <input type="hidden" id="tour_id_{!! $row->id !!}"
                                                           value="{!! $row->id !!}">
                                                    <td id="price_{{$row->id}}">{{ $row->price }}</td>
                                                    <td>
                                                        <a href="javascript:void(0);"
                                                           onclick="generateSingleInvoice('{{ $row->id }}')"
                                                           class="btn-sm btn btn-outline-primary">{{__("messages.generate_invoice")}}</a>
                                                    </td>
                                            ...
                                            @endforeach
                                            <tr>
                                                <td colspan="10">{{$rows->appends(request()->input())->links()}}</td>
                                            </tr>
